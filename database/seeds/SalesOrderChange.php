...
class SalesOrderChange extends Seeder
{
    /**
    ...
    public function run()
    {
        $so = DB::table('sales_orders')->all();
        foreach($so as $sales_order){
            $products = [];
            foreach($so->products as $so_product){
                $products[] = [
            ...
            DB::table('sales_orders')->where('_id', $sales_order->id)->update(['products' => $products]);
        }
    }
}
