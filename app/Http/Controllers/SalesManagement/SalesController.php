...
class SalesController extends Controller
{
    ...
    {
        $this->middleware('perm.acc:sales');
    }

    //Public index master-user
    public function index()
    {
        return view('panel.sales-management.sales.index');
    }
    ...
    public function find(Request $request)
    {
        ...
        if ($request->id) {
            $user = User::where('email', $request->email)->where('role', 'elemMatch', array('name' => 'Sales'))->first();
            if (count($user) > 0) {
            ...
            return (User::where('email', $request->email)->where('role', 'elemMatch', array('name' => 'Sales'))->first() ? 'false' : 'true');
        }
    }
    ...
    public function create()
    {
        ...
        $roles = Role::where('name', 'Sales')->get();
        return view('panel.sales-management.sales.form-create')->with([
            'list_ap' => $list_ap,
            'list_mp' => $list_mp,
            'roles' => $roles,
        ]);
    ...
    {
        $users = User::where('role', 'elemMatch', array('name' => 'Sales'))->get();
        ...
        return Datatables::of($users)
            ...
            ->addColumn('action', function ($user) {
                return
                '<a class="btn btn-success btn-sm" href="' . route('sales-staff.edit', ['id' => $user->id]) . '">
						<i class="fa fa-pencil-square-o"></i>&nbsp;Edit</a>' .
                '<form style="display:inline;" method="POST" action="' .
                route('sales-staff.destroy', ['id' => $user->id]) . '">' . method_field('DELETE') . csrf_field() .
    ...
    public function edit($id)
    {
        ...
        $list_ap = (Auth::user()->email == env('ROOT_USERNAME') ? Auth::user()->accPermissions() : Auth::user()->accessPermissions);
        $list_mp = (Auth::user()->email == env('ROOT_USERNAME') ? Auth::user()->modPermissions() : Auth::user()->modulePermissions);
        $roles = Role::where('_id', '<>', $user->role[0]['_id'])->where('name', 'Sales')->get();
        ...
        return view('panel.sales-management.sales.form-edit')->with([
            'user' => $user,
            'list_ap' => $list_ap,
            'list_mp' => $list_mp,
            'roles' => $roles,
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('sales-staff.index')->with('dlt', 'sales');
    }
}
