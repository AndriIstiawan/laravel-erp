...
class ProductionsController extends Controller
{
    ...
    public function find(Request $request)
    {
        ...
        if ($request->id) {
            $user = User::where('email', $request->email)->where('role', 'elemMatch', array('name' => 'Production'))->first();
            if (count($user) > 0) {
            ...
            return (User::where('email', $request->email)->where('role', 'elemMatch', array('name' => 'Production'))->first() ? 'false' : 'true');
        }
    }
    ...
    public function create()
    {
        ...
        return view('panel.producer-management.producer.form-create')->with([
            'list_ap' => $list_ap,
            'list_mp' => $list_mp,
            'roles' => $roles,
        ]);
    ...
    public function edit($id)
    {
        ...
        $list_ap = (Auth::user()->email == env('ROOT_USERNAME') ? Auth::user()->accPermissions() : Auth::user()->accessPermissions);
        $list_mp = (Auth::user()->email == env('ROOT_USERNAME') ? Auth::user()->modPermissions() : Auth::user()->modulePermissions);
        $roles = Role::where('_id', '<>', $user->role[0]['_id'])->where('name', 'Production')->get();
        ...
        return view('panel.producer-management.producer.form-edit')->with([
            'user' => $user,
            'list_ap' => $list_ap,
            'list_mp' => $list_mp,
            'roles' => $roles,
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('productions-staff.index')->with('dlt', 'sales');
    }
}
