												...
												<div class="form-group">
													...
													<select id="role" class="form-control" style="width: 100% !important;" name="role"
														...
														<option value="{{$role->id}}"
															data-description="{{$role->description}}" selected="">{{$role->name}}</option>
														@endforeach
													</select>
	...
	$("#jxForm1").validate({
		rules:{
			...
			email:{
				...
				email:true,
				remote:{
					url: '{{ route('sales-staff.index') }}/find',
					type: "post",
					data:{
						_token:'{{ csrf_token() }}',
						id: $('.id').val(),
		...
		messages:{
			name:{
				required:'Please enter a name sales',
	...
	function act(action){
		switch(action) {
		    case 'continue':
		        ...
		        break;
		    case 'new':
		        window.open("{{ route('sales-staff.create') }}/?new=sales-staff", "_self");
		        break;
		    case 'exit':
		        window.open("{{ route('sales-staff.index') }}/?new=sales-staff", "_self");
		}
	}
