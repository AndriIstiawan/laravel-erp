                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="dd" id="nestable3">
                           <?php
                           function nested($listArr, $variants){
                                ...
                                <li class="dd-item dd3-item" data-slug="{{$la['slug']}}" data-name="{{$la['name']}}">
                                    ...
                                    <?php
                                        $like = $la['slug'];
                                        $result = array_filter($variants, function($item) use ($like){
                                            foreach($item['parent'] as $ip){
                                                if($ip == $like){
                                                    return true;
                                                }
                                            }
...
<script>
    function save(act) {
        ...
        $.ajax({
            ...
            url: "{{ route('variant.index') }}/reorder-update",
            type: 'GET',
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                setTimeout(function () {
                    ...
                    if (act == 'continue') {
                        ...
                        window.open("{{ route('variant.index') }}/?edit=categories", "_self");
                    }
                }, {{ env('SET_TIMEOUT', '500') }});
            },
            error: function (e) {}
