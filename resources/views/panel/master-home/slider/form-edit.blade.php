												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" class="form-control" id="name" name="name" placeholder="Name"
															aria-describedby="name-error" value="{{ $slider->name }}">
														<em id="name-error" class="error invalid-feedback">Please enter a name of slider </em>
													</div>
													<div class="text-center">
														<img class="rounded picturePrev" 
															src="{{(isset($slider->picture)?asset('img/slider/'.$slider->picture):asset('img/fiture-logo.png'))}}" 
...
<script>
	...
	function postData(formAct1,formAct2,action,sendForm){
		...
		$.ajax({
			url : "{{ route('slider.update',['id' => $slider->id]) }}",
			type: 'POST',
			processData: false,
        	contentType: false,
